#include "ec.h"
#include "field.h"
#include "e0_basis.h"// TODO: get rid of that ugly thing
#include <assert.h>

uint32_t
ec_recover_y(field_t *y, const field_t *Px, const ec_curve_t *curve)
{ // Recover y-coordinate of a point on the Montgomery curve y^2 = x^3 + Ax^2 + x
    field_add(y, Px, &curve->A); // A + x
    field_mul(y, y, Px);         // x^2 + Ax
    field_add_one(y, y);         // x^2 + Ax + 1
    field_mul(y, y, Px);         // x^3 + Ax^2 + x
    // This is required, because we do not yet know that our curves are
    // supersingular so our points live on the twist with B = 1.
    return field_sqrt_verify(y);
}
...
{
    assert(field_is_one(&B->P.z));
    assert(field_is_one(&E->C));

    field_copy(&P->x, &B->P.x);
    field_copy(&Q->x, &B->Q.x);
    field_copy(&Q->z, &B->Q.z);
    field_set_one(&P->z);
    uint32_t ret = ec_recover_y(&P->y, &P->x, E);

    // Algorithm of Okeya-Sakurai to recover y.Q in the montgomery model
    field_t v1, v2, v3, v4;
    field_mul(&v1, &P->x, &Q->z);
    field_add(&v2, &Q->x, &v1);
    field_sub(&v3, &Q->x, &v1);
    field_sqr(&v3, &v3);
    field_mul(&v3, &v3, &B->PmQ.x);
    field_add(&v1, &E->A, &E->A);
    field_mul(&v1, &v1, &Q->z);
    field_add(&v2, &v2, &v1);
    field_mul(&v4, &P->x, &Q->x);
    field_add(&v4, &v4, &Q->z);
    field_mul(&v2, &v2, &v4);
    field_mul(&v1, &v1, &Q->z);
    field_sub(&v2, &v2, &v1);
    field_mul(&v2, &v2, &B->PmQ.z);
    field_sub(&Q->y, &v3, &v2);
    field_add(&v1, &P->y, &P->y);
    field_mul(&v1, &v1, &Q->z);
    field_mul(&v1, &v1, &B->PmQ.z);
    field_mul(&Q->x, &Q->x, &v1);
    field_mul(&Q->z, &Q->z, &v1);

    // Transforming to a jacobian coordinate
    field_sqr(&v1, &Q->z);
    field_mul(&Q->y, &Q->y, &v1);
    field_mul(&Q->x, &Q->x, &Q->z);
    return ret;
}
...
lift_basis(jac_point_t *P, jac_point_t *Q, ec_basis_t *B, ec_curve_t *E)
{
    ...
    field_t inverses[2];
    field_copy(&inverses[0], &B->P.z);
    field_copy(&inverses[1], &E->C);

    field_batched_inv(inverses, 2);
    field_set_one(&B->P.z);
    field_set_one(&E->C);

    field_mul(&B->P.x, &B->P.x, &inverses[0]);
    field_mul(&E->A, &E->A, &inverses[1]);
...
static uint32_t
is_on_curve(const field_t *x, const ec_curve_t *curve)
{
    assert(field_is_one(&curve->C));
    field_t t0;

    field_add(&t0, x, &curve->A); // x + (A/C)
    field_mul(&t0, &t0, x);       // x^2 + (A/C)*x
    field_add_one(&t0, &t0);      // x^2 + (A/C)*x + 1
    field_mul(&t0, &t0, x);       // x^3 + (A/C)*x^2 + x

    return field_is_square(&t0);
}
...
{
#ifdef FP_ONLY
    printf("Function ec_curve_to_basis_2f_to_hint is not implemented with Fp-only arithmetic.\n");
    assert(0);
#else
...
{
#ifdef FP_ONLY
    printf("Function ec_curve_to_basis_2f_from_hint is not implemented with Fp-only arithmetic.\n");
    assert(0);
#else
    // Normalise (A/C : 1) and ((A + 2)/4 : 1)
    ec_normalize_curve_and_A24(curve);
