...
{ // Swap points in constant time
  // If option = 0 then P <- P and Q <- Q, else if option = 0xFF...FF then P <- Q and Q <- P
    field_cswap(&(P->x), &(Q->x), option);
    field_cswap(&(P->z), &(Q->z), option);
}
...
{ // Neither the curve or A24 are guaranteed to be normalized.
  // First we normalize (A/C : 1) and conditionally compute
    ...
    if (!E->is_A24_computed_and_normalized) {
        ...
        field_add_one(&E->A24.x, &E->A);     // A24.x = A + 1
        field_add_one(&E->A24.x, &E->A24.x); // A24.x = A + 2

        field_half(&E->A24.x, &E->A24.x); // (A + 2) / 2
        field_half(&E->A24.x, &E->A24.x); // (A + 2) / 4
        field_set_one(&E->A24.z);
...
{
    return field_is_zero(&P->x) | field_is_zero(&P->z);
}
...
{ // Evaluate if two points in Montgomery coordinates (X:Z) are equal
  // Returns 0xFFFFFFFF (true) if P=Q, 0 (false) otherwise
    ...
    uint32_t lr_equal = field_is_equal(&t0, &t1);
...
ec_is_two_torsion(const ec_point_t *P, const ec_curve_t *E)
{
    ...
    field_mul(&t1, &t1, &E->C);
    field_add(&t1, &t1, &t1);
    field_add(&t0, &t1, &t2); // 4 (CX^2+CZ^2+AXZ)

    x_is_zero = field_is_zero(&P->x);
    tmp_is_zero = field_is_zero(&t0);
...
int
ec_curve_init_from_A(ec_curve_t *E, const field_t *A)
{ // Initialize the curve from the A coefficient and check it is valid
  // Return 1 if curve is valid, 0 otherwise
    ec_curve_init(E);
    field_copy(&E->A, A); // Set A
    return ec_curve_verify_A(A);
}

void
ec_j_inv(field_t *j_inv, const ec_curve_t *curve)
{ // j-invariant computation for Montgommery coefficient A2=(A+2C:4C)
    field_t t0, t1;

    field_sqr(&t1, &curve->C);
    field_sqr(j_inv, &curve->A);
    field_add(&t0, &t1, &t1);
    field_sub(&t0, j_inv, &t0);
    field_sub(&t0, &t0, &t1);
    field_sub(j_inv, &t0, &t1);
    field_sqr(&t1, &t1);
    field_mul(j_inv, j_inv, &t1);
    field_add(&t0, &t0, &t0);
    field_add(&t0, &t0, &t0);
    field_sqr(&t1, &t0);
    field_mul(&t0, &t0, &t1);
...
{ // Doubling of a Montgomery point in projective coordinates (X:Z). Computation of coefficient values A+2C and 4C
  ...
  // Output: projective Montgomery x-coordinates Q <- 2*P = (XQ:ZQ) such that x(2P)=XQ/ZQ.
    ...
    field_sqr(&t1, &t1);
    field_sub(&t2, &t0, &t1);
    field_add(&t3, &AC->z, &AC->z);
    field_mul(&t1, &t1, &t3);
    field_add(&t1, &t1, &t1);
    field_mul(&Q->x, &t0, &t1);
    field_add(&t0, &t3, &AC->x);
    field_mul(&t0, &t0, &t2);
    field_add(&t0, &t0, &t1);
    field_mul(&Q->z, &t0, &t2);
}
...
{ // Differential addition of Montgomery points in projective coordinates (X:Z).
  ...
  // Output: projective Montgomery point R <- P+Q = (XR:ZR) such that x(P+Q)=XR/ZR.
    ...
    field_sqr(&t2, &t2);
    field_sqr(&t3, &t3);
    field_mul(&t2, &PQ->z, &t2);
    field_mul(&R->z, &PQ->x, &t3);
    field_copy(&R->x, &t2);
}
...
{ // Simultaneous doubling and differential addition.
  ...
  //         x(Q+P)=XS/ZS.
    ...
    field_sub(&t1, &P->x, &P->z);
    field_sqr(&R->x, &t0);
    field_sub(&t2, &Q->x, &Q->z);
    field_add(&S->x, &Q->x, &Q->z);
    field_mul(&t0, &t0, &t2);
    field_sqr(&R->z, &t1);
    field_mul(&t1, &t1, &S->x);
    field_sub(&t2, &R->x, &R->z);
    if (!A24_normalized)
        field_mul(&R->z, &R->z, &A24->z);
    field_mul(&R->x, &R->x, &R->z);
    field_mul(&S->x, &A24->x, &t2);
    field_sub(&S->z, &t0, &t1);
    field_add(&R->z, &R->z, &S->x);
    field_add(&S->x, &t0, &t1);
    field_mul(&R->z, &R->z, &t2);
    field_sqr(&S->z, &S->z);
    field_sqr(&S->x, &S->x);
    field_mul(&S->z, &S->z, &PQ->x);
    field_mul(&S->x, &S->x, &PQ->z);
}
...
void xTPL(ec_point_t* Q, const ec_point_t* P, const ec_point_t* A3)
{
    ...
    field_sub(&t0, &P->x, &P->z);
    field_sqr(&t2, &t0);
    field_add(&t1, &P->x, &P->z);
    field_sqr(&t3, &t1);
    field_add(&t4, &t1, &t0);
    field_sub(&t0, &t1, &t0);
    field_sqr(&t1, &t4);
    field_sub(&t1, &t1, &t3);
    field_sub(&t1, &t1, &t2);
    field_mul(&Q->x, &t3, &A3->x);
    field_mul(&t3, &Q->x, &t3);
    field_mul(&Q->z, &t2, &A3->z);
    field_mul(&t2, &t2, &Q->z);
    field_sub(&t3, &t2, &t3);
    field_sub(&t2, &Q->x, &Q->z);
    field_mul(&t1, &t2, &t1);
...
{ // The Montgomery ladder
  ...
  // Output: projective Montgomery points Q <- k*P = (XQ:ZQ) such that x(k*P)=XQ/ZQ.
    ...
    if (!curve->is_A24_computed_and_normalized) {
        ...
        field_add(&A24.x, &curve->C, &curve->C);
        field_add(&A24.z, &A24.x, &A24.x);
        field_add(&A24.x, &A24.x, &curve->A);
    } else {
        field_copy(&A24.x, &curve->A24.x);
        field_copy(&A24.z, &curve->A24.z);
...
{ // The Montgomery biladder
  ...
  // Output: projective Montgomery point S <- k*P + l*Q = (XS:ZS) such that x(k*P + l*Q)=XS/ZS.
    ...
    field_copy(&DIFF1a.x, &R[1].x);
    field_copy(&DIFF1a.z, &R[1].z);
    field_copy(&DIFF1b.x, &R[2].x);
    field_copy(&DIFF1b.z, &R[2].z);
    ...
    field_copy(&DIFF2a.x, &R[2].x);
    field_copy(&DIFF2a.z, &R[2].z);
    field_copy(&DIFF2b.x, &PQ->x);
    field_copy(&DIFF2b.z, &PQ->z);

    A_is_zero = field_is_zero(&curve->A);
    ...
    for (i = kbits - 1; i >= 0; i--) {
        ...
        if (A_is_zero) {
            ...
            assert(field_is_one(&curve->A24.z));
...
ec_mul(ec_point_t *res, const digit_t *scalar, const int kbits, const ec_point_t *P, ec_curve_t *curve)
{
    ...
    // When A24 is computed and normalized we save some field multiplications
                ...
                const ec_curve_t *curve)
{
    ...
    if (kbits == 1) {
        ...
        if (!field_is_zero(&curve->A)) { // If A is not zero normalize
