 ...
 */
int ec_curve_verify_A(const field_t *A);
 ...
 * @return 0  if curve is invalid, 1 otherwise
 */
int ec_curve_init_from_A(ec_curve_t *E, const field_t *A);
...
{
    field_copy(&(E1->A), &(E2->A));
    field_copy(&(E1->C), &(E2->C));
...
AC_to_A24(ec_point_t *A24, const ec_curve_t *E)
{
    ...
    // A24 = (A+2C : 4C)
    field_add(&A24->z, &E->C, &E->C);
    field_add(&A24->x, &E->A, &A24->z);
    field_add(&A24->z, &A24->z, &A24->z);
}
...
{
    // (A:C) = ((A+2C)*2-4C : 4C)
    field_add(&E->A, &A24->x, &A24->x);
    field_sub(&E->A, &E->A, &A24->z);
    field_add(&E->A, &E->A, &E->A);
    field_copy(&E->C, &A24->z);
}
 ...
 */
void ec_j_inv(field_t *j_inv, const ec_curve_t *curve);
 ...
 */
uint32_t ec_recover_y(field_t *y, const field_t *Px, const ec_curve_t *curve);
...
void jac_to_ws(jac_point_t *P, field_t *t, field_t *ao3, const jac_point_t *Q, const ec_curve_t *curve);
void jac_from_ws(jac_point_t *Q, const jac_point_t *P, const field_t *ao3, const ec_curve_t *curve);
...
void DBLW(jac_point_t *Q, field_t *u, const jac_point_t *P, const field_t *t);
